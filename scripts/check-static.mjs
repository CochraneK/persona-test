...
for (const marker of requiredPlayMarkers) {
  if (!play.includes(marker)) errors.push(`play shell missing marker: ${marker}`);
}
...
if (fs.existsSync(imgDir)) {
  ...
  for (const file of files) {
    ...
    if (buf.length > 1024 * 1024) warnings.push(`large image >1MB: ${file} (${(buf.length / 1024 / 1024).toFixed(2)} MB)`);
  }
